...
const Header = ({ showSearchBar = true, showCart = true }) => {
  ...
  useEffect(() => {
    console.log("User:", user); // Verifica el estado de `user` al montar el componente

    if (user) {
      GetUserInfo()
        .then((data) => {
          console.log("User Info Data:", data); // Verifica los datos recibidos de `GetUserInfo`
          setUserInfo(data);
        })
        .catch((error) => console.error("Error fetching user info:", error));
    } else {
      console.log("No user, clearing userInfo");
      setUserInfo(null); // Clear userInfo if not logged in
    }
  }, [user]);
          ...
          {user ? (
            <div className="flex items-center space-x-4">
              <span className="hover:text-gray-400">
                Bienvenido, {userInfo?.firstname || user.firstname || "Usuario"}
              </span>
              <button
                onClick={() => {
                  logout();
                  window.location.href = "/login"; // Redirige al login después de logout
                }}
